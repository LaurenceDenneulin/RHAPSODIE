...
for k in range(-3, 0, step=0.5)
    ...
    files = readdir(joinpath(base_path, "rhapsodie_method_results", "max_iter_700"))
    for file in files
        if endswith(file, ".fits")
            ...
            x_est = read_and_fill_polar_map(parameter_type, joinpath(base_path, "rhapsodie_method_results", "max_iter_700", file))
